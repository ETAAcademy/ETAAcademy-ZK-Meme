...
- **Version A**:
  ...
  1. The prover selects a random number r and computes R = rG.
  2. The prover then calculates the challenge $c = \text{hash}(PK, R)$.
  3. The response is $z = r + c \cdot sk$.
  4. The verifier checks that $c = \text{hash}(PK, R)$.

- **Version B**:
  1. The prover selects a random number r and computes R = rG.
  2. The prover then calculates the challenge $c = \text{hash}(PK, R)$ and sends both $z = r + c \cdot sk$ and c as the response.
  3. The verifier computes $R = zG - cPK$ and checks that $c = \text{hash}(PK, R)$.
