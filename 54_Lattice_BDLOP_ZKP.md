...
1. **Encryption Process:**
   The scheme is based on a plaintext space (ring $R_p$) and a noise distribution $\chi$. Encryption uses the public key and secret key to encrypt the message. The public key, $pk$, consists of a matrix
    ...
    <div  align="center"> 
    <img src="images/54_matrix01.png" width="20%" />
    </div>
    
   where $a$ and $b$ are generated by multiplying random vectors with the noise distribution $\chi$. When encrypting a message $m$ with a random vector $\vec{r} = (r_1, r_2, r_3) \leftarrow \chi^{3d}$, the ciphertext is computed as:

    <div  align="center"> 
    <img src="images/54_matrix_02.png" width="30%" />
    </div>
