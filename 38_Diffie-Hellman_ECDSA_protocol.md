...
To verify the authenticity of the shared secret, the protocol incorporates zero-knowledge proofs. Both parties employ zk-Schnorr proofs to demonstrate knowledge of their respective private keys without revealing them, i.e., $x_1$, generating $proof_1$, sending $(proof_1, Q_1)$, and verifying $(proof_2, Q_2)$, and vice versa.

Additionally, zk-Paillier-N proofs are used to certify the validity of the Paillier key pairs involved in potential encryption schemes (pk, sk), meaning the public key $N$ and private key $\phi(N)$ are coprime,i.e., $gcd(N, \phi(N)) = 1$.
