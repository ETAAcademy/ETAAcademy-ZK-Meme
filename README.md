...
Groth16: upgrades Pinocchio, also verifies bilinearpairing $e([A]_1,[B]_2) = e(αG_1, βG_2) \cdot e (∑^l_{i=0} \frac{βu_i(x)+αv_i(x)+w_i(x)}/{γ} G_1, γG_2) ⋅ e([C]_1, δG_2)$ by QAP, $∑^m_{i=0}a_iu_i(x)∑^m_{i=0}a_iv_i(x)=∑^m_{i=0}a_iω_i(x) + h(x)z(x),$ but less constraints.
